...
def DBN_synthetic_generation_example(images):
	...
	dbn = Pre_trainer_synthetic_generator(n_visible=images_reshaped.shape[1], hidden_array=[625*3, 484*3, 400*3], epochs=300, optim='adm', k=20)
...
def generate_class_wise(path):
	...
	for file_path in [path+i for i in os.listdir(path)]:
		...
		synthetic = DBN_synthetic_generation_example(data)
...
def generate_complete_dataset(path):
	...
	synthetic = DBN_synthetic_generation_example(dataset)
	tots = 0
